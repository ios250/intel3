...
RUN echo '123123' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/123123.sh

RUN echo 'cd ' >>/123123.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1152x864' "  >>/123123.sh

RUN echo 'cd /noVNC-1.2.0' >>/123123.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/123123.sh

RUN chmod 755 /123123.sh
